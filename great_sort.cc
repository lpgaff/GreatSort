// ============================================================================================= //
/*! \mainpage A general purpose sorting code for data in MIDAS' GREAT format
* **Author**: Liam Gaffney [(liam.gaffney@liverpool.ac.uk)](mailto:liam.gaffney@liverpool.ac.uk), University of Liverpool
*
* See the [GitHub page](https://github.com/lpgaff/GreatSort) for installation
* instructions, to submit bug reports, and to understand the sorting philosophy.
*/
// ============================================================================================= //
...
// Settings file
std::shared_ptr<GreatSettings> myset;

// Calibration file
std::shared_ptr<GreatCalibration> mycal;
bool overwrite_cal = false;

// Reaction file
std::shared_ptr<GreatReaction> myreact;
...
std::shared_ptr<GreatConverter> conv_mon;
std::shared_ptr<GreatEventBuilder> eb_mon;
std::shared_ptr<GreatHistogrammer> hist_mon;
...
void* monitor_run( void* ptr ){
	...
	conv_mon = std::make_shared<GreatConverter>( calfiles->myset );
	eb_mon = std::make_shared<GreatEventBuilder>( calfiles->myset );
	hist_mon = std::make_shared<GreatHistogrammer>( calfiles->myreact, calfiles->myset );
...
void start_http(){
	...
	// Server for JSROOT
	std::string server_name = "http:" + std::to_string(port_num) + "?top=GreatDAQMonitoring";
...
void do_hist(){
	...
	//------------------------------//
	GreatHistogrammer hist( myreact, myset );
...
int main( int argc, char *argv[] ){
	...
	myreact = std::make_shared<GreatReaction>( name_react_file, myset, flag_source );
